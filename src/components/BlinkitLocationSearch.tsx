...
import { 
  searchBlinkitLocation, 
  confirmBlinkitLocation, 
  BlinkitLocationSuggestion 
...
export function BlinkitLocationSearch() {
  ...
  const placeholders = [
    "Enter your location (e.g., Badarpur)",
  ...
  const handleSelectLocation = async (suggestion: BlinkitLocationSuggestion) => {
    ...
    try {
      const data = await confirmBlinkitLocation(
        suggestion.meta.place_id,
        suggestion.title.text,
        suggestion.subtitle.text,
        suggestion.meta.session_token
      );
      
      if (data.success && data.locationData) {
        // Check if location is serviceable
        if (data.locationData.is_serviceable && data.locationData.is_available) {
          // Success: Store validation and navigate to product search page
          const validatedLocations: Array<{location: string, timestamp: number}> = JSON.parse(sessionStorage.getItem('blinkitValidatedLocations') || '[]');
          ...
          updatedValidations.push({
            location: data.locationData.display_address.address_line,
            timestamp: Date.now()
          });
          sessionStorage.setItem('blinkitValidatedLocations', JSON.stringify(updatedValidations));
          
          // Navigate with locationData encoded in URL
          const locationDataParam = encodeURIComponent(JSON.stringify(data.locationData));
          router.push(`/blinkitSearchResults?location=${encodeURIComponent(data.locationData.display_address.address_line)}&locationData=${locationDataParam}`);
          setSuggestions([]); // Hide dropdown after selection
        } else {
          // Location not serviceable
          setError('Sorry, Blinkit doesn\'t deliver to this location yet');
        }
      } else {
